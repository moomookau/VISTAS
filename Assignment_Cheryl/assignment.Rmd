...
*Excel: Industry Skills Needs* is downloaded from <https://datacatalog.worldbank.org/dataset/skills-linkedin-data>. We will be using **public_use-industry-skills-needs.xlsx**.
...
-   `readxl`: to load data from excel file to R
...
packages = c('readr', 'readxl', 'dplyr', 'tidyr', 'DT', 'formattable', 'reactable',
...
### Loading the data with read_excel

We use `read_excel` to load the csv file.

```{r}
industryskillsneeds <- read_excel("../data/public_use-industry-skills-needs.xlsx", sheet="Industry Skills Needs")
```

Then we use `str()` to view the structure of the data. Since we want the shift in industry skill needs over the years, we have to pivot our data to have year as columns and skill_group_name as values.
...
![Dashboard with full width](images/storyboard1.png){width="80%"}
